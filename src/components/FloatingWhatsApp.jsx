...
const FloatingWhatsApp = () => {
  ...
  return (
    <div className="fixed z-50 bottom-6 right-6 flex items-center gap-3">
      {/* Tooltip */}
      {showTooltip && (
        <div className="hidden md:block glass backdrop-blur-xl bg-white/95 text-textPrimary px-5 py-3 rounded-2xl shadow-strong border border-primary/20 animate-fade-in-left">
      ...
      {/* Botão WhatsApp com design moderno */}
      <a
        href={`https://wa.me/${phoneNumber}?text=${whatsappMessage}`}
        target="_blank"
        rel="noopener noreferrer"
        aria-label="Abrir WhatsApp"
        className={`group relative bg-[#25D366] text-white w-16 h-16 rounded-2xl shadow-strong hover:shadow-glow-primary flex items-center justify-center transition-all duration-300 hover:scale-110 hover:-translate-y-1 ${bounce ? 'animate-bounce-slow' : ''}`}
      ...
      >
        ...
        {/* Ícone WhatsApp */}
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-8 h-8 relative z-10 group-hover:scale-110 transition-transform">
        ...
        <span className="absolute -top-1 -right-1 w-4 h-4 bg-cta rounded-full border-2 border-white animate-pulse"></span>
      </a>
    </div>
  );
};
